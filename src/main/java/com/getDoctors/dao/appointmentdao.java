...
public class appointmentdao {
	...
	public boolean AddAppointment(String name, String email, String date, String time, String note, int drid, int ptid,
			int dsid) {

		try {
			Connection conn = conObj.dbconnect();
			PreparedStatement ps = conn.prepareStatement(
					"INSERT INTO `appointment`(`patient_name`, `patient_email`, `appointment_date`, `appointment_time`, `additional_note`, `doctor_id`, `patient_id`, `disease`) VALUES (?,?,?,?,?,?,?,?)");
			ps.setString(1, name);
	...
	public List<appointment> PatientsAppointments(int id) {
		List<appointment> data = new ArrayList<>();
		try {
			Connection conn = conObj.dbconnect();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM `appointment` where patient_id = ?;");
			ps.setInt(1, id);
	...
	public boolean approveAppoitment(int id) {
		...
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("UPDATE `appointment` SET appointment_status = 1 where appointment_id = ?");
			ps.setInt(1, id);
	...
	public boolean rejectAppoitment(int id) {
		...
		try {
			Connection conn = conObj.dbconnect();
			PreparedStatement ps = conn
					.prepareStatement("UPDATE `appointment` SET appointment_status = 2 where appointment_id = ?");
			ps.setInt(1, id);
